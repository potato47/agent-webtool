...
export function normalizeAndValidateUrl(input: string): URL {
  ...
  if (url.protocol !== 'http:' && url.protocol !== 'https:') {
    throw new WebtoolError(
      'invalid_url',
      `Only http(s) is supported, got ${url.protocol}`,
    )
  }
...
export async function ensurePublicHost(host: string): Promise<void> {
  ...
  if (isPrivateAddress(host)) {
    throw new WebtoolError(
      'private_address',
      `Host ${host} is a private/reserved address`,
    )
  }
  try {
...
export async function fetchWithGuards(
  ...
  const timeoutId = setTimeout(() => ctrl.abort(new WebtoolError('timeout', `Timed out after ${timeoutMs}ms`)), timeoutMs)
