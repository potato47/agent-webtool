...
describe("isSameOriginRedirect", () => {
  test("strips leading www and matches", () => {
    expect(
      isSameOriginRedirect(new URL("https://example.com/a"), new URL("https://www.example.com/b")),
    ).toBe(true);
  });
  test("rejects protocol change", () => {
    expect(isSameOriginRedirect(new URL("https://x.com/"), new URL("http://x.com/"))).toBe(false);
  });
  test("rejects different host", () => {
    expect(isSameOriginRedirect(new URL("https://a.com/"), new URL("https://b.com/"))).toBe(false);
  });
});
